...
$list.Add_DrawColumnHeader({
    ...
    # Always fill header background (light gray for light theme, dark gray for dark)
    $headerColor = if ($form.BackColor -eq [System.Drawing.Color]::Black) { [System.Drawing.Color]::DimGray } else { [System.Drawing.Color]::LightGray }
    $e.Graphics.FillRectangle((New-Object System.Drawing.SolidBrush $headerColor), $e.Bounds)
...
$list.Add_DrawItem({
    ...
    # Remove hover effect: do not change background on hover
    $isEven = $e.ItemIndex % 2 -eq 0
    $rowBack = if ($form.BackColor -eq [System.Drawing.Color]::Black) {
        if ($isEven) { [System.Drawing.Color]::Black } else { [System.Drawing.Color]::FromArgb(30,30,30) }
    } else {
        if ($isEven) { [System.Drawing.Color]::White } else { [System.Drawing.Color]::FromArgb(245,245,245) }
    }
...
$list.Add_DrawSubItem({
    ...
    $bounds = [System.Drawing.Rectangle]$bounds

    $textBrush = if ($form.BackColor -eq [System.Drawing.Color]::Black) { [System.Drawing.Brushes]::Aquamarine } else { [System.Drawing.Brushes]::Purple }

    # Draw text with padding, no hover effect
    $e.Graphics.DrawString([string]$e.SubItem.Text, $fontRow, $textBrush, [float]($bounds.X + 5), [float]($bounds.Y + 3))
})
