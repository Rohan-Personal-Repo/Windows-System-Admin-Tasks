...
# ---------- Draw Headers ----------
$list.OwnerDraw = $true   # keep this once

$list.Add_DrawColumnHeader({
    param($sender, $e)

    # fixed header color, not based on form backcolor
    $headerColor = [System.Drawing.Color]::FromArgb(64,64,64)
    $e.Graphics.FillRectangle(
        (New-Object System.Drawing.SolidBrush $headerColor),
        $e.Bounds
    )
    ...
    # white header text so it is visible on dark gray
...
$list.Add_DrawItem({
    ...
    # row background based on current theme, but NOT changing theme itself
    $isDarkTheme = ($form.BackColor -eq [System.Drawing.Color]::Black)
    ...
    $e.Graphics.FillRectangle(
        (New-Object System.Drawing.SolidBrush $rowBack),
        $e.Bounds
    )
    $e.DrawDefault = $false
})
...
$list.Add_DrawSubItem({
    ...
    $textBrush = if ($e.Item.Selected) {
        [System.Drawing.Brushes]::White
    } else {
        if ($isDarkTheme) { [System.Drawing.Brushes]::Aquamarine }
        else              { [System.Drawing.Brushes]::Purple }
    }
...
$form.Add_Shown({
    ...
    Load-List $AllRows
    Apply-DarkTheme      # start in dark theme if you like
})

$txtSearch.Add_TextChanged({
    $filter = $txtSearch.Text.Trim()
    if ($filter) { Load-List (Get-ActorRows $filter) } else { Load-List $AllRows }
})
