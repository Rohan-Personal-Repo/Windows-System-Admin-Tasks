...
function updateMultipleValues {
    ...
    # 2. Find existing actor document by actorName and get its _id
    $existing = Get-MdbcData -Filter @{ actorName = $actorName } -First 1 -As PS

    if ($existing) {

        $id = $existing._id   # this is a MongoDB ObjectId

        Write-Host "Found existing actor '$actorName' with _id $id. Appending new role..." -ForegroundColor Cyan

        # 3a. Actor exists -> use _id in filter and $push into roles[]
        Update-MdbcData `
            -Filter @{ _id = $id } `
