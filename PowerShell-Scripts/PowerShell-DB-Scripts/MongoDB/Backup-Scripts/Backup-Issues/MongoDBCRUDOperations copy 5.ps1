...
function updateMultipleValues {
    ...
    # Ensure roles is an array if some documents have object
    $fixRolesCmd = "db.ActorsData.updateMany(
        { actorName: '$actorNameSafe', roles: { \$type: 'object' } },
        [ { \$set: { roles: [ '\$roles' ] } } ]
    )"
    mongosh $connectionString --quiet --eval $fixRolesCmd

    # Update actorShowName and push new role
    $cmd = "db.ActorsData.updateOne(
        { actorName: '$actorNameSafe' },
        {
