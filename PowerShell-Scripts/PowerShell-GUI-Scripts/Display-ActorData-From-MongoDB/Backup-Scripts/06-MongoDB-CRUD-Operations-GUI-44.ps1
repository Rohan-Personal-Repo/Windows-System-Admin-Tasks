...
# Enable themed headers / visual styles
...
$list.Font          = $fontRow
$list.OwnerDraw     = $false        # keep default, safe drawing [web:189][web:237]
# ---------- Row spacing / taller rows ----------
# $imgList = New-Object System.Windows.Forms.ImageList
# $imgList.ImageSize = New-Object System.Drawing.Size(1, 32)  # height controls row height (e.g. 28px)
# $list.SmallImageList = $imgList

# Add Column Sizing
$null = $list.Columns.Add("actorName",       360)  # was 320
$null = $list.Columns.Add("roleName",        360)
$null = $list.Columns.Add("seriesMovieName", 580)
$null = $list.Columns.Add("roleAddress",     420)

# after creating columns
$actorCol   = $list.Columns[0]
$roleCol    = $list.Columns[1]
$seriesCol  = $list.Columns[2]
$addressCol = $list.Columns[3]
...
function Apply-LightTheme {
    ...
    # headers: text color follows theme
    $actorCol.ForeColor   = [System.Drawing.Color]::Purple
    $roleCol.ForeColor    = [System.Drawing.Color]::Purple
    $seriesCol.ForeColor  = [System.Drawing.Color]::Purple
    $addressCol.ForeColor = [System.Drawing.Color]::Purple
}
...
function Apply-DarkTheme {
    ...
    # headers: text color follows theme
    $actorCol.ForeColor   = [System.Drawing.Color]::Aquamarine
    $roleCol.ForeColor    = [System.Drawing.Color]::Aquamarine
    $seriesCol.ForeColor  = [System.Drawing.Color]::Aquamarine
    $addressCol.ForeColor = [System.Drawing.Color]::Aquamarine
}
...
function Load-List {
    ...
    foreach ($col in $list.Columns) {
        $col.Width = -2   # -2 = autosize to header + content [web:286][web:288]
    }
