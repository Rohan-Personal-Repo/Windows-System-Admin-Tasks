...
# ---------- OwnerDraw: DrawSubItem ----------
$list.Add_DrawSubItem({
    param($sender, $e)

    # Ensure $bounds is a single rectangle
    $bounds = $e.Bounds
    if ($bounds -is [System.Array]) { $bounds = $bounds[0] }

    # Fallback in case still not a Rectangle
    ...
    # Draw string using X/Y coordinates directly (simpler, avoids PointF issues)
