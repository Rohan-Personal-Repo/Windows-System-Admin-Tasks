...
# ---------- OwnerDraw: DrawItem (background/selection) ----------
$list.Add_DrawItem({
    param($sender, $e)
    # Highlight selected item
    if ($e.Item.Selected) {
        $backColor = if ($form.BackColor -eq [System.Drawing.Color]::Black) {
...
$list.Add_DrawColumnHeader({
    ...
    # Header background
    $headerBack = if ($form.BackColor -eq [System.Drawing.Color]::Black) {
        [System.Drawing.Brushes]::DimGray
    } else {
        [System.Drawing.Brushes]::LightGray
    }
    $e.Graphics.FillRectangle($headerBack, $e.Bounds)
...
$list.Add_DrawSubItem({
    ...
    # If selected, use system highlight text
    if ($e.Item.Selected) {
