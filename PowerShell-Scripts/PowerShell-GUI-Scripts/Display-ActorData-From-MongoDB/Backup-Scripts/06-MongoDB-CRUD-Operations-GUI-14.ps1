...
function Get-FlattenedActors {
    ...
    # return as plain array so indexing always works
    return $list.ToArray()
}
...
# ---------- State ----------
[object[]]$allRows    = Get-FlattenedActors
[int]$allIndex        = 0
[object[]]$searchRows = @()
[int]$searchIndex     = 0

$allIndex = Show-RowOnLabels $allRows $allIndex `
    $lblAllHeroVal $lblAllRoleVal $lblAllSeriesVal $lblAllAddrVal $lblAllInfo
...
$btnSearch.Add_Click({
    ...
    if ($searchRows.Count -eq 0) {
        ...
        $searchIndex = Show-RowOnLabels $searchRows $searchIndex `
            $lblS_HeroVal $lblS_RoleVal $lblS_SeriesVal $lblS_AddrVal $lblS_Info
    }
})
