...
function updateMultipleValues {
    ...
    # 2. Check if an actor document already exists (match on actorName only)
    $existing = Get-MdbcData -Filter @{ actorName = $actorName } -First 1 -As PS

    if ($existing) {
        # 3a. Actor exists -> append new role to roles[] using $push
        #     This does NOT overwrite existing array elements.
        Update-MdbcData `
            -Filter @{ actorName = $actorName } `
            -Update @{ '$push' = @{ roles = $newRole } }
    }
    else {
        # 3b. Actor does not exist -> create a new document with roles as array
        $doc = [PSCustomObject]@{
            actorName = $actorName
            roles     = @($newRole)
        }
