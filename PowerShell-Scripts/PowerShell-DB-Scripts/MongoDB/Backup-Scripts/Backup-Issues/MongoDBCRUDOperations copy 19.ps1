...
# --------------------------
# Normalize roles to arrays if object
# --------------------------
...
db.ActorsData.insertOne({
    ...
    "actorShowName": "' + $heroNameSafe + '",
    "roles": [{ "heroName": "' + $heroNameSafe + '", "seriesMovieTitle": "' + $movieSafe + '", "location": "' + $locationSafe + '" }]
})
'@
    mongosh $connectionString --quiet --eval $cmd
...
# 1. Normalize roles first
Normalize-Roles

# 2. Insert actor
createActor -actorName "Salma Hayek" -heroName "Sonia Kincaid" -movie "Hitmans Wifes Bodyguard" -location "Ottawa"

# 3. Read specific actor
