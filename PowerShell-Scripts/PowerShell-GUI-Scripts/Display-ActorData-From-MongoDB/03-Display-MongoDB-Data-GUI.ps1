...
# Import Module MDBC and stop if it isn't installed
Import-Module Mdbc -ErrorAction Stop

[System.Windows.Forms.Application]::EnableVisualStyles()

# We assume Data already resides on MongoDB Collection

# ---------- MongoDB ----------
$mongoServer    = $env:MONGO_CONN_STRING
# If if the Environment Variable is Set
if (-not $mongoServer) {
...
# wider base widths
$null = $list.Columns.Add("actorName",       360)
...
function Load-List {
    ...
    # auto-size columns to header + content so long names fit
    for ($i = 0; $i -lt $list.Columns.Count; $i++) {
        $list.AutoResizeColumn($i, [System.Windows.Forms.ColumnHeaderAutoResizeStyle]::HeaderSize)
