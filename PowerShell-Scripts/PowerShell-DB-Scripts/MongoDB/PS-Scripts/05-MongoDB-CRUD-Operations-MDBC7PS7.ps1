...
# Connected to the Database and Collection created using MongoDB Compass
...
# Append role or create actor : Looks up the actor by heroName
...
function updateMultipleValues {
    param(
        [Parameter(Mandatory)][string]$heroName,          # e.g. "Kate Beckinsale"
        [Parameter(Mandatory)][string]$roleName,          # e.g. "Selene"
        [Parameter(Mandatory)][string]$seriesMovieTitle,  # e.g. "Underworld"
        [Parameter(Mandatory)][string]$Address            # e.g. "Budapest"
    )

    # 1. Build new role object in the final schema
    ...
    # 2. Look for existing document for this heroName
    $existing = Get-MdbcData -Filter @{ heroName = $heroName } -First 1 -As PS

    if ($existing) {

        # Use _id to be absolutely sure we target this document
        $id = $existing._id

        Write-Host "Found existing hero '$heroName' with _id $id. Appending role..." -ForegroundColor DarkGreen -BackgroundColor Black

        # 3a. Append new role to roles[] with $push
        Update-MdbcData `
            -Filter @{ _id = $id } `
        ...
        Write-Host "Hero '$heroName' not found. Creating new document..." -ForegroundColor Yellow

        # 3b. Create new document with roles array
        $doc = [PSCustomObject]@{
            heroName = $heroName
            roles    = @($newRole)
        }
...
# Insert actor
updateMultipleValues -heroName "Salma Hayek" `
            -roleName "Sonia Kincaid" `
...
Write-Host "`n--- Updating Multiple Values ---" -ForegroundColor Magenta -BackgroundColor Black
updateMultipleValues -heroName "Matt Leblanc" `
                     -roleName "Adam Burns" `
