...
function updateMultipleValues {
    ...
    # 2. Get the existing actor document (if any) by actorName
    $existing = Get-MdbcData -Filter @{ actorName = $actorName } -First 1 -As PS

    if ($existing) {
        # We have a single, concrete document with a specific _id
        $id = $existing._id

        Write-Host "Found existing actor '$actorName' with _id $id. Appending role..." -ForegroundColor Cyan

        # 3a. Append new role to roles array for THIS _id only
        Update-MdbcData `
            -Filter @{ _id = $id } `
        ...
        # 3b. Actor does not exist -> create new document with roles as array
        $doc = [PSCustomObject]@{
            actorName = $actorName
            roles     = @($newRole)
        }
