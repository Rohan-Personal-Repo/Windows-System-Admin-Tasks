...
#     # Get all documents as PS objects
#     $actors = Get-MdbcData -As PS

#     if (-not $actors) {
#         Write-Host "No documents found in collection." -ForegroundColor Yellow
#         return
#     }

#     $rows = foreach ($actor in $actors) {

#         # Normalize roles to an array (handles 1 or many)
...
#     $rows | Out-GridView -Title "Actors and Roles"
# }
...
function updateMultipleValues {
    ...
    # Build the new role object exactly as stored in Mongo
    ...
    # Check if actor document exists (match on actorName only)
    $existing = Get-MdbcData -Filter @{ actorName = $actorName } -First 1 -As PS

    if ($existing) {
        # Append the new role to roles[]; do NOT use $set or positional "$"
        Update-MdbcData `
            -Filter @{ actorName = $actorName } `
            -Update @{ '$push' = @{ roles = $newRole } }
    }
    else {
        # Create a completely new actor document with roles as array
        $doc = [PSCustomObject]@{
            actorName = $actorName
            roles     = @($newRole)
        }
