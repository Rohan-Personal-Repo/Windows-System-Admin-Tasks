...
function updateMultipleValues {
    param(
        [Parameter(Mandatory)][string]$actorName,        # actor (e.g. "Matt Leblanc")
        [Parameter(Mandatory)][string]$heroName,         # new role name  (e.g. "Adam Burns")
        [Parameter(Mandatory)][string]$seriesMovieTitle, # new show/movie (e.g. "Man with a Plan")
        [Parameter(Mandatory)][string]$location          # new location   (e.g. "Vancouver")
    )
    ...
    # Does the actor document already exist?
    $existing = Get-MdbcData -Filter @{ actorName = $actorName } -First 1 -As PS

    if ($existing) {
        # Actor exists -> append new role to roles array
        Update-MdbcData `
            -Filter @{ actorName = $actorName } `
            -Update @{ '$push' = @{ roles = $newRole } }
    }
    else {
        # Actor does not exist -> create full new document
        $doc = [PSCustomObject]@{
            actorName = $actorName
            roles     = @($newRole)
        }
