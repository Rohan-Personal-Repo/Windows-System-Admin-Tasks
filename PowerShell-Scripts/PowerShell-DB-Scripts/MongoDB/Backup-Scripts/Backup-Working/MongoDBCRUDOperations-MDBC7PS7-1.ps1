...
function readOneActor {
    ...
    $actor = Get-MdbcData -Query @{ heroName = $actorName }

    if (-not $actor) {
        Write-Host "Actor '$actorName' not found."
    }
    else {
        $actor | Format-List | Out-String | Write-Host
    }
}
...
function readAllActors {
    ...
    if (-not $actors) {
        ...
        $actors | ForEach-Object {
            Write-Host "Actor: $($_.heroName)"
            $_.roles | ForEach-Object {
                Write-Host "  Role : $($_.roleName) | Show/Movie: $($_.seriesMovieTitle) | Location: $($_.Address)"
            }
            Write-Host ""
        }
    }
